...
if(isset($_GET['id']) && $_GET['id'] > 0){
    ...
    if(isset($student_id)){
     ...
     if(isset($month)){
        $monthl = $conn->query("SELECT * FROM month_list where id = '{$month}'");
        if($monthl->num_rows > 0){
            $res = $monthl->fetch_array();
            $month_Name = $res['months'];
        }
    }
}
?>
        ...
        <div class="card card-outline card-primary shadow rounded-0">
            ...
            <div class="card-body rounded-0">
                <div class="container-fluid">
                    ...
                    <small class="text-muted">Submitted by <b class="text-info"><?= $firstname.' '.$lastname ?></b> on  <?= date("F d, Y h:i A",strtotime($date_created)) ?></small>
                    ...
                    <fieldset>
                         <legend class="text-navy">Project Adviser:</legend>
                        <div class="pl-4"><large><?= isset($adviser) ? $adviser : "----" ?></large></div>
                    </fieldset>
                     <fieldset>
                        <legend class="text-navy">Project Month - Year:</legend>
                        <div class="pl-4"><large><?= isset($year) ? isset($month) ? $month_Name.'  '.$year :"" : "----" ?></large></div>
                    </fieldset>
                    <fieldset>

...
if(isset($_GET['id']) && $_GET['id'] > 0){
    $qry = $conn->query("SELECT *,month_list.months as months FROM `archive_list` INNER JOIN month_list ON month_list.id = archive_list.month where archive_list.id = '{$_GET['id']}'");
    if($qry->num_rows){
            ...
            <div class="container-fluid">
                <form action="" id="archive-form">
                    ...
                    <div class="row">
                          <div class="col-lg-3">
                            <div class="form-group">
                                <label for="month" class="control-label text-navy">Month</label>
                                <select name="month" id="month" class="form-control form-control-border" required>
                                   <?php isset($months) ? '<option value="'.$month.'">'.$months.'</option>' : "" ?>
                                  <?php 
                                        $month = $conn->query("SELECT * FROM `month_list` order by `id` asc");
                                        while($row = $month->fetch_assoc()):
                                        ?>
                                        <option value="<?= $row['id'] ?>"><?= ucwords($row['months']) ?></option>
                                        <?php endwhile; ?>
                                </select>
                            </div>
                        </div>
                        ...
                        <div class="col-lg-6">
                            <div class="form-group">
                                <label for="year" class="control-label text-navy">Project Adviser</label>
                                <input type="text" name="adviser" id="adviser" placeholder="Project Adviser" class="form-control form-control-border" value="<?= isset($adviser) ?$adviser : "" ?>">
                            </div>
                        </div>
                    </div>
                    <div class="row">
